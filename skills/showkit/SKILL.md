---
name: showkit
description: Create and check a guided HTML interactive demo from a verified OpenAI app browser session, bound static source, or an isolated Playwright flow.
---
...
or Claude app, a codebase or static build, or an isolated Playwright flow.
...
Playwright test. Let the host agent handle the CLI and ask only for the output
folder, source access, private-content choice, and any external permission.
...
- Claude built-in Chrome extraction is unavailable whenever the installed
  `javascript_tool` capability runs in page context and does not provide a
  host-validated isolated page world. Read
  `agents/claude-browser.md` and return `UnsupportedSurface` before using that
  tool for extraction or persistence.
- In Claude Code, the Claude Desktop Code tab, or another terminal-capable
  host, offer the optional codebase-free headed Chrome flow in
  `examples/headed-chrome-live.md` when the person wants live capture and
  approves Playwright. The fresh non-persistent context requires sign-in in
  that temporary window and never copies an existing browser profile.
...
1. Read `workflow.md`, `references/security.md`, and
   `references/visual-fidelity.md`.
2. Run `showkit doctor --capability playwright --json`.
...
5. Run `showkit capture <demo.spec.ts> --json`.
6. Read only the saved CaptureSource evidence needed to write the StorySpec. Do
   not inspect browser storage, headers, cookies, or raw product data.
7. Keep tooltip claims within captured visible text and action outcomes.
8. Default `player.chrome.mode` to `overlay` and fill the embed container.
...
9. Limit card backdrop values to `off`, `light`, `medium`, or `heavy`. Set the
...
10. Before the first StorySpec apply, show the exact theme defaults and ask whether to keep them
...
11. Require every player theme to pass ShowKit's WCAG 2.2 AA player checks:
...
12. Run `showkit story apply <story.json> --json`.
13. Run `showkit validate --json`.
14. Run `showkit build web,markdown --json`.
15. Run `showkit preview --json`, state that the URL is local and not
...
16. Present the first local preview with its current backdrop, navigation,
...
17. Include the default centered completion lead card with title **Ready to create your demo?**,
...
18. When an earlier manifest is available, run
